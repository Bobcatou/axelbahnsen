				...
				<?php for ($i=1; $i <= 3; $i++) { ?>
					<?php query_posts('page_id=' . get_pageId(html_entity_decode(get_option('chameleon_home_page_'.$i)))); while (have_posts()) : the_post(); ?>
						<?php 
							global $more; $more = 0;
						?>
						<?php $et_service_link = get_post_meta($post->ID,'etlink',true) ? get_post_meta($post->ID,'etlink',true) : get_permalink(); ?>
<div class="service<?php if ( $i == 3 ) echo ' last'; ?>">
<h3 class="title"><a href="<?php echo $et_service_link; ?>"><?php the_title(); ?></a></h3>							
							<?php
								$thumb = '';
								$width = 232;
								$height = 117;
